...
func (s *Server) loadBoardForm(db serverDB, r *http.Request, b *Board) {
	...
	b.MinSizeThread = max(int64(FormFloat64(r, "minsizethread")*1000000), 0)
	b.MaxSizeThread = max(int64(FormFloat64(r, "maxsizethread")*1000000), 0)
	b.MinSizeReply = max(int64(FormFloat64(r, "minsizereply")*1000000), 0)
	b.MaxSizeReply = max(int64(FormFloat64(r, "maxsizereply")*1000000), 0)
...
func (s *Server) serveBoard(data *templateData, db serverDB, w http.ResponseWriter, r *http.Request) (skipExecute bool) {
	...
	if modBoard != "" {
		...
		if len(data.Threads) > 0 && len(data.Threads[0]) > 0 && data.Threads[0][0] != nil && data.Threads[0][0].Archived() {
