...
func (s *Server) importPosts(sqlDB *sql.DB, table string, tinyIB bool, commit bool) (int, error) {
	// Build query.
	var query string
	if tinyIB {
...
func (s *Server) serveImport(data *templateData, db *database.DB, w http.ResponseWriter, r *http.Request) {
	...
	data.Template = "manage_info"
	data.Message += `<b>Warning:</b> Backup all files and databases before importing a board.<br><br>`
	...
	for _, info := range s.importDatabases {
		...
		if table == "" {
			data.Message += template.HTML(fmt.Sprintf("<b>Error:</b> Failed to locate post table in export %s", html.EscapeString(info.name)))
			return
		}
		postCount, err := s.importPosts(sqlDB, table, tinyIB, false)
		if err != nil {
			data.Message += template.HTML(fmt.Sprintf("<b>Error:</b> Failed to query post table in export %s: %s", html.EscapeString(info.name), html.EscapeString(err.Error())))
			return
		} else if postCount == 0 {
			data.Message += template.HTML(fmt.Sprintf("<b>Error:</b> No posts were found in export %s.", html.EscapeString(info.name)))
			return
		}
	...
	if !commit {
		// TODO show dropdown for import mapping
