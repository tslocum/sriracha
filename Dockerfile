...
# Compile sriracha.
RUN cd /usr/src/sriracha/cmd/sriracha && go build -trimpath -x -ldflags "-s -w -X 'codeberg.org/tslocum/sriracha.SrirachaVersion=${version:-DEV}'"

# Compile plugins.
RUN for dir in /usr/src/sriracha/plugin/*; do (cd "$dir" && go build -trimpath -x -ldflags "-s -w" -buildmode=plugin); done
